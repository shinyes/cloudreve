...
export const processTaskContent = (summary: TaskSummary, userHashId: string): TaskSummary => {
  // `summary.props` can be null at runtime (Go marshals a nil map as null), so read
  // through a local. Optional chaining must not appear on the left of an assignment.
  const props = summary.props;

  if (props?.src) {
    props.src = processUrl(props.src, userHashId);
  }
  if (props?.dst) {
    props.dst = processUrl(props.dst, userHashId);
  }
  if (props?.src_multiple) {
    props.src_multiple = props.src_multiple.map((url) => processUrl(url, userHashId));
  }
