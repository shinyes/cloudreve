...
export const getActionOpt = (
  ...
  display.showCreateArchive =
    ...
    display.orCapability &&
    display.orCapability.enabled(NavigatorCapability.download_file);
  // Relocating only makes sense for the user's own files and folders; whether a
  // second policy exists is filtered in the dialog (policy list is user-scoped).
  display.showRelocate = display.hasReadable && !!currentUser && targets.every((t) => t.owned !== false);
