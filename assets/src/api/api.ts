...
import {
  ...
  AvailablePolicyListResponse,
  AvailableStoragePolicy,
  CreateFileService,
  ...
  FileURLService,
  GetFileInfoService,
  ListAvailablePolicyService,
  ListFileService,
  ListResponse,
  MoveFileService,
  MultipleUriService,
  PatchMetadataService,
  PatchPreferredPolicyService,
...
/**
 * Lists the storage policies granted to the current user. When a URI is given,
 * the response also carries the policy currently preferred for that folder.
 */
export function getAvailablePolicies(args: ListAvailablePolicyService): ThunkResponse<AvailablePolicyListResponse> {
  return async (dispatch, _getState) => {
    return await dispatch(
      send(
        `/user/policies`,
        { method: "GET", params: { uri: args.uri } },
        {
          ...defaultOpts,
        },
      ),
...
/** Sets, or clears (empty policy_id), the storage policy preferred for a folder. */
export function sendPatchPreferredPolicy(args: PatchPreferredPolicyService): ThunkResponse<void> {
  return async (dispatch, _getState) => {
    return await dispatch(
      send(
        `/user/preferredPolicy`,
        { method: "PATCH", data: args },
        {
          ...defaultOpts,
        },
...
/** Creates a task that moves the selected files or folders to another storage policy. */
export function sendRelocate(args: RelocateWorkflowService): ThunkResponse<TaskResponse> {
  return async (dispatch, _getState) => {
    return await dispatch(
      send(
        `/workflow/relocate`,
