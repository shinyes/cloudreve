...
import { GroupEnt, GroupStoragePolicy } from "../../../../api/dashboard";
...
const BasicInfoSection = () => {
  ...
  const onPolicyChange = useCallback(
    (value: GroupStoragePolicy[]) => {
      setGroup((p: GroupEnt) => ({
        ...p,
        edges: { ...p.edges, storage_policies: value },
      }));
    },
    [setGroup],
  );
        ...
        {values?.id != AnonymousGroupID && (
          <>
            <SettingForm title={t("group.availablePolicies")} lgWidth={5}>
              <PolicySelectionInput value={values.edges.storage_policies ?? []} onChange={onPolicyChange} />
