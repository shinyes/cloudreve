...
/** A storage policy the current user may pick for a folder. */
export interface AvailableStoragePolicy {
  id: string;
  name: string;
  type: string;
  max_size: number;
...
export interface ListAvailablePolicyService {
  uri?: string;
}

export interface AvailablePolicyListResponse {
  policies: AvailableStoragePolicy[];
  /** Policy explicitly preferred for the queried folder, if any. */
  preferred_policy?: string;
  /** Policy used when the folder has no preference: the first granted policy. */
  default_policy?: string;
}

export interface PatchPreferredPolicyService {
  uri: string;
