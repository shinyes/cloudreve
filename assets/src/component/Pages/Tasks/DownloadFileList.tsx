...
const DownloadFileList = ({ taskId, summary, downloading, readonly }: DownloadFileListProps) => {
  ...
  // The backend marshals a nil map as null, so `summary.props` may be null even
  // though the declared type says otherwise. `?? {}` keeps this a real null check
  // that the minifier cannot reduce back to `summary.props.download`.
  const summaryProps = summary?.props ?? {};
  const files = summaryProps.download?.files;
