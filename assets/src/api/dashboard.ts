...
/**
 * A storage policy as it appears on a group's `storage_policies` edge.
 * Requests only need `id`; responses also carry `name`.
 */
...
export interface GroupEnt extends CommonMixin {
  ...
  permissions?: string;
  edges: {
    // A group can be granted multiple storage policies; members may pick any of
    // them per folder. The first entry is the fallback policy.
    storage_policies?: GroupStoragePolicy[];
  };
  total_users?: number;
